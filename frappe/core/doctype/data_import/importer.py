...
class Column:
	...
	def validate_values(self):
		...
		if self.df.fieldtype == "Link":
			...
			values = set([cstr(v) for v in self.column_values[1:] if v])
			exists = [
				d.name for d in frappe.db.get_all(self.df.options, filters={"name": ("in", list(values))})
			]
			not_exists = list(values - set(exists))
			if not_exists:
